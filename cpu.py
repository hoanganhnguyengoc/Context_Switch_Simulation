...
class PIDmanager:
    ...
    def createPid(self, process):
        ...
        """
        if self.max_cur == self.max:
        # Nếu đã đạt đến giới hạn PID, hãy tìm PID không sử dụng
            for index, used in enumerate(self.used_pid):
                if used == 0:
                    process.pid = index
                    self.used_pid[index] = 1
                    break
        else:
        # Nếu chưa đạt đến giới hạn, hãy sử dụng max_cur làm PID cho tiến trình mới
            process.pid = self.max_cur
            self.max_cur += 1
